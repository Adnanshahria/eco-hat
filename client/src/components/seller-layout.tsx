...
import {
    LayoutDashboard, ShoppingCart, Package, DollarSign, User, LogOut, Store, Menu, X
} from "lucide-react";
            ...
            <aside className="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-50 lg:flex lg:w-64 lg:flex-col">
                <div className="flex grow flex-col gap-y-5 overflow-y-auto border-r bg-card px-6 pb-4">
                    ...
                    <nav className="flex flex-1 flex-col">
                        <ul className="flex flex-1 flex-col gap-y-1">
                            {navItems.map((item) => {
                                const isActive = location === item.href ||
                                    (item.href !== "/seller" && location.startsWith(item.href));
                                return (
                                    <li key={item.href}>
                                        <Link href={item.href}>
                        ...
                        <Button variant="ghost" onClick={signOut} className="mt-auto justify-start gap-2 text-destructive hover:text-destructive hover:bg-destructive/10">
                    ...
                    <div className="fixed inset-y-0 left-0 w-64 bg-card p-6">
                        <div className="flex items-center justify-between mb-6">
                            <span className="font-bold text-lg flex items-center gap-2">
                                <Store className="h-5 w-5 text-primary" /> Seller Portal
                            </span>
